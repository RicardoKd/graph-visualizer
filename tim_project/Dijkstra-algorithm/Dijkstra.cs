...
namespace tim_project.Dijkstra_algorithm {
  ...
  public static class Dijkstra { //: IDijkstra {
    ...
    public static string[] Dijkstra_algorithm_for_one_element(Graph graph, GraphVertex start)//Dijkstra's algorithm for one vertex
  {
      ...
      for (int i = 0; i < graph.Vertices.Count; i++) {
        if (i == ind_start) { arr[i] = new dij_element(0, start.Name, true); kolvo_true++; prev = 0; } else
    ...
    public static Graph graph_of_min_cost(Graph graph) { //graph of minimal cost
